		...
		case $DESK in
			...
			"Debian GNU/Linux 10 (buster)")
				...
				if [[ $x == 'q' ]]; then
					...
					{ sudo apt install -y git wget libpixman-1-dev python flex coreutils binwalk qemu-user libtool wget python build-essential libtool-bin bison libglib2.0-dev libglib2.0 2>&- && echo -e "${INFO}Installing dependencies...${NC}"; } || { echo -e "${ERROR}Uh oh... issue installing dependencies....${NC}" && exit 1; }
				fi
				;;
			*)
...
extract () {
	...
	sudo binwalk -e /usr/share/focs/$(basename "$file")  || { echo -e "${ERROR}Unfortunately, binwalk threw an issue... This can't be fixed by me, I'm afraid...${NC}" && exit 1; }; 
